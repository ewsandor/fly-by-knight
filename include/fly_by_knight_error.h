...
#define FBK_ASSERT_MSG(exp, msg, ...) if(!(exp)) {FBK_ERROR_MSG_HARD(msg, ##__VA_ARGS__); fbk_close_log_file(); assert(exp); }
#define FBK_FATAL_MSG_CODE(code, msg, ...) FBK_ERROR_MSG_HARD(msg, ##__VA_ARGS__); fbk_close_log_file(); fbk_exit(code);
#define FBK_FATAL_MSG(msg, ...) FBK_FATAL_MSG_CODE(1, msg, ##__VA_ARGS__)
